    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        if batch_id <= configs.num_save_samples:
            ...
            for i in range(configs.total_length):
                ...
                name = 'unit8_gt' + str(i + 1) + '.png'
                file_name = os.path.join(path, name)
                img_gt_unit8 = np.uint8(test_ims[0, i, :, :, :] * 255)
                cv2.imwrite(file_name, img_gt_unit8)
            ...
            for i in range(output_length):
                ...
                name = 'unit8_pd' + str(i + 1 + configs.input_length) + '.png'
                file_name = os.path.join(path, name)
                img_pd_unit8 = np.uint8(img_pd * 255)
                cv2.imwrite(file_name, img_pd_unit8)
